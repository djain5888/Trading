...
Obtains an access token and refreshes it before expiry. When no API secret is
configured the API key is used directly as the bearer token (no exchange).
Token exchange goes through the shared HTTP client, so it inherits retries,
timeouts and error mapping. Access is serialised with a lock to avoid a refresh
stampede. Time comes from an injected :class:`Clock` — never ``datetime.now``.
"""
